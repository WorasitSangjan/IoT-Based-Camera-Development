...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("xgboost_prediction.log"),
...
def main():
    try:
        # Load dataset
        data_path = r'/Users/ws/Library/CloudStorage/OneDrive-Personal/Paper_IoT/Spring_Wheat/Output/2_segmented_data.csv'
        ...
        for loop in range(10):
            ...
            for interval in sorted(pivot_data['interval'].unique()):
                ...
                for train_idx, test_idx in loo.split(X):
                    ...
                    model = XGBRegressor()  # No random_state for variation between loops
        ...
        summary_metrics.to_csv('aggregated_interval_metrics_xgb.csv')
        logging.info("Aggregated results saved to 'aggregated_interval_metrics_xgb.csv'")

        # Save aggregated feature importance
        final_feature_importance = pd.concat(overall_feature_importance, ignore_index=True)
        final_feature_importance.to_csv('aggregated_feature_importance_xgb.csv', index=False)
        logging.info("Aggregated feature importance saved to 'aggregated_feature_importance_xgb.csv'")
        ...
        plt.tight_layout()
        plt.savefig('metrics_with_uncertainty_xgb.png')
        plt.close()
